...
{
    if ( ($_SESSION['debug'] ?? false) || ($GLOBALS['debug'] ?? false)|| ($_REQUEST['debug'] ?? false) || (getenv('DEBUG') ?? false) ) 
	{
...
{
	if ( ( $_SESSION['verbose'] ?? false ) || ( $GLOBALS['verbose'] ?? false )|| ( $_REQUEST['verbose'] ?? false ) || ( getenv('VERBOSE') ?? false ) ) 
    {
        if ('cli' === PHP_SAPI ) {
            fprintf( STDERR, "- %s%s\n", ($extend ?? '?') , $str );
        } else {
